...
def parse_args(isLocal=False):
	...
	parser.add_argument("--conda-env", help="The name of the conda/mamba environment to activate.", required=True)
	parser.add_argument("--use-mamba", help="Use mamba instead of conda.", action="store_true", default=False)
...
"""
def submit_local(snakefile, profile, htcondor_jobdir, conda_env, use_mamba):
	...
	# Build the arguments string with conda env info
	args_str = f"long --snakefile {snakefile} --profile {profile} --htcondor-jobdir {htcondor_jobdir} --conda-env {conda_env}"
	if use_mamba:
		args_str += " --use-mamba"
	...
	submit_description = htcondor.Submit({
		...
		# modules we've installed in the submission environment.
		"getenv":                  "true",

		"JobBatchName":            f"sm-manager-process-{time.strftime('%Y%m%d-%H%M%S')}",
	})
...
def top_main():
	...
	try:
		submit_local(args.snakefile, args.profile, args.htcondor_jobdir, args.conda_env, args.use_mamba)
	except Exception as e:
		print(f"Error: Could not submit local universe job. {e}")
		raise

"""
Command to activate conda environment and run Snakemake at the AP. This is run by the local universe job, not the user.
"""
def long_main():
	args = parse_args(True)
	
	# Determine which package manager to use
	pkg_manager = "mamba" if args.use_mamba else "conda"
	...
	#
	# For mamba, we still need to source conda's initialization script
	# but we use mamba for activation
	# For mamba, we need to use 'mamba shell hook' instead of sourcing conda.sh
    # For conda, we source the conda.sh script as before
	if args.use_mamba:
		init_command = 'eval "$(mamba shell hook --shell bash)"'
	else:
		init_command = 'source $(conda info --base)/etc/profile.d/conda.sh'

	command = f"""
	{init_command} && \
	{pkg_manager} activate {args.conda_env} && \
